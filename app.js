...
const sourceDriver=document.getElementById("sourceDriver"),sourceTransport=document.getElementById("sourceTransport"),manualDt=document.getElementById("manualDt"),manualPlaces=document.getElementById("manualPlaces"),manualWeight=document.getElementById("manualWeight"),manualDo=document.getElementById("manualDo"),manualRef=document.getElementById("manualRef"),applicationSources=document.getElementById("applicationSources");
let mode="",blobUrl=null;

function applicationData(){
  return Заявка.parseSources(sourceDriver?.value||"",sourceTransport?.value||"",{
    places:manualPlaces?.value||"",weight:manualWeight?.value||"",do:manualDo?.value||"",dt:manualDt?.value||"",ref:manualRef?.value||""
  });
}
function hasApplicationData(){return [sourceDriver,sourceTransport,manualDt,manualPlaces,manualWeight,manualDo,manualRef].some(e=>e&&String(e.value||"").trim())}
...
[sourceDriver,sourceTransport,manualDt,manualPlaces,manualWeight,manualDo,manualRef].forEach(e=>e&&e.addEventListener("input",()=>{if(mode==="заявка"){if(!document.getElementById("parsedFields").hidden)stats.textContent="Данные изменены — нажмите «Обновить распознанные данные»";else updateApplicationState()}}));
...
clear.onclick=()=>{
  if(mode==="заявка"){[sourceDriver,sourceTransport,manualDt,manualPlaces,manualWeight,manualDo,manualRef].forEach(e=>e.value="");document.getElementById("parsedFields").hidden=true;status.innerHTML="";updateApplicationState()}
