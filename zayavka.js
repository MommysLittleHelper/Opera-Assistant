...
const Заявка = (() => {
  ...
  function parse(t){
    ...
    // Структурное распознавание свободного текста: ключевые слова не обязательны.
    // ФИО — 2 или 3 русских слова непосредственно перед паспортом.
    if(!d.driver){
      m=x.match(/\b([А-ЯЁ][а-яё-]+\s+[А-ЯЁ][а-яё-]+(?:\s+[А-ЯЁ][а-яё-]+)?)\s*(?=,?\s*(?:ПАСПОРТ\b|\d{4}\s+\d{6}\b))/i);
      if(m)d.driver=clean(m[1]);
    }
    // Паспорт — 4 цифры серии + 6 цифр номера. Дата рядом с паспортом.
    if(!d.passportSeries || !d.passportNumber){
      m=x.match(/\b(?:ПАСПОРТ\s*)?(\d{4})\s+(\d{6})\b/i);
      if(m){d.passportSeries=m[1];d.passportNumber=m[2]}
    }
    if(!d.passportDate && d.passportNumber){
      const pi=x.indexOf(d.passportNumber);
      const tail=pi>=0?x.slice(pi+d.passportNumber.length):'';
      m=tail.match(/\b(\d{2}\.\d{2}\.\d{4})\b/);
      if(m)d.passportDate=m[1];
    }
    // Госномер — российский формат. Он может быть без метки «Госномер».
    if(!d.plate){
      m=x.match(/\b[А-ЯЁA-Z]\s*\d{3}\s*[А-ЯЁA-Z]{2}\s*\d{2,3}\b/i);
      if(m)d.plate=m[0].replace(/\s+/g,'').toUpperCase();
    }
    // Кем выдан — текст после даты паспорта до автомобиля/госномера/телефона.
    if(!d.issuedBy && d.passportDate){
      const di=x.indexOf(d.passportDate);
      if(di>=0){
        const tail=x.slice(di+d.passportDate.length);
        const stops=[tail.search(/\b(?:ГАЗ|GAZ|КАМАЗ|МАЗ|MAN|VOLVO|SCANIA|DAF|IVECO|MERCEDES|RENAULT|FORD|FAW|HOWO|SHACMAN)\b/i), d.plate?tail.toUpperCase().indexOf(d.plate.toUpperCase()):-1, tail.search(/(?:\+7|8)\s*\(?\d{3}/)].filter(i=>i>=0);
        const e=stops.length?Math.min(...stops):tail.length;
        d.issuedBy=clean(tail.slice(0,e)).replace(/[;,]+$/,'');
      }
    }
    // Автомобиль — фрагмент перед госномером; если есть марка, начинаем с неё.
    if(!d.car && d.plate){
      const pi=x.toUpperCase().indexOf(d.plate.toUpperCase());
      if(pi>=0){
        const before=x.slice(0,pi).trim();
        const vm=before.match(/(?:ГАЗ|GAZ|КАМАЗ|МАЗ|MAN|VOLVO|SCANIA|DAF|IVECO|MERCEDES|RENAULT|FORD|FAW|HOWO|SHACMAN)\b.*$/i);
        if(vm)d.car=clean(vm[0]).replace(/^ГАЗ\s+АВТОМОБИЛЬ\s+/i,'').replace(/^GAZ\s+АВТОМОБИЛЬ\s+/i,'');
        else if(d.issuedBy){
          const vi=before.lastIndexOf(d.issuedBy);
          if(vi>=0)d.car=clean(before.slice(vi+d.issuedBy.length));
        }
      }
    }
    ...
    // Получатель: строго после «П/П». Берём до следующего реквизита или закрывающей скобки.
    let recipient="";
    let rm=x.match(/\bП\/П\s+(.+?)(?=\)|\s+(?:ИНВ(?:ОЙС)?\b|ТД\b|ДТ\b|ДО\b|ЖДН\b|CMR\b)|$)/i);
    if(rm) recipient=clean(rm[1]).replace(/[\s),.;:]+$/g,"").trim();
    if(!recipient){
      rm=x.match(/(?:в\s+адрес\s*:\s*|в\s+адрес\s+получателя\s+)(.+?)(?=\)|\s+(?:ИНВ(?:ОЙС)?\b|ТД\b|ДТ\b)|$)/i);
      if(rm) recipient=clean(rm[1]).replace(/^АО\s*[«"“]?ЛОГИСТИКА[-–—\s]+ТЕРМИНАЛ[»"”]?\s*/i,"").replace(/^П\/П\s*/i,"").replace(/[\s),.;:]+$/g,"").trim();
    }
    d.recipient=recipient;

    // Инвойс: номер после ИНВ/ИНВОЙС до ТД. Дата инвойса не сохраняем.
    m=x.match(/\bИНВ(?:ОЙС)?\s*(?:№\s*)?([0-9A-ZА-ЯЁ][0-9A-ZА-ЯЁ\/-]*)\b(?:\s+от\s+\d{2}\.\d{2}\.\d{4})?(?=\s*(?:ТД\b|ДТ\b)|[\s,;]|$)/i);
    if(!m)m=x.match(/\bИНВ(?:ОЙС)?\s*(?:№\s*)?([0-9A-ZА-ЯЁ\/-]+)\b/i);
    d.invoice=m?m[1]:"";
    d.invoiceDate="";

    // ДТ теперь берётся только из ручного поля, а не из свободного текста.
    d.dt="";
  ...
  function parseSources(driverText, transportText, manual={}) {
    ...
    if (manual.dt) d.dt=String(manual.dt).trim();
